...
class ItemDatabaseHelper(context: Context):SQLiteOpenHelper(context, DB_NAME,null,1) {
    ...
    companion object{
        ...
        const val TABLE_ITEMS = "subjects"
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        val createItems = "create table $TABLE_ITEMS($COLUMN_ID integer primary key autoincrement ,$COLUMN_SUBJECT_NAME varchar(30) not null,$COLUMN_ATTENDED integer default 0,$COLUMN_TOTAL integer default 0)"
    ...
    fun markAttendance(id:Int,attended:Boolean = false){
        val db = this.writableDatabase
        try {
            val query = if (attended) "update $TABLE_ITEMS set $COLUMN_ATTENDED = $COLUMN_ATTENDED+1,$COLUMN_TOTAL = $COLUMN_TOTAL+1 where $COLUMN_ID = $id"
            else "update $TABLE_ITEMS set $COLUMN_TOTAL = $COLUMN_TOTAL+1 where $COLUMN_ID = $id"
            db.execSQL(query)
    ...
    fun deleteItem(id: Int){
        val query = "delete from $TABLE_ITEMS where $COLUMN_ID = $id"
        try {
    ...
    fun getItem(id: Int): Item {
        val query = "select * from $TABLE_ITEMS where $COLUMN_ID = $id"
